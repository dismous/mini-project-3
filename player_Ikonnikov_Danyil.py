...
def valid_borders(field: list, figure: list, coords: tuple, friendly: str, enemy: str):
...
def valid_placement(field: list, figure: list, coords: tuple, friendly: str, enemy: str):
    ...
    :param field list: 2D array representing the game field
    :param figure list: 2D array representing the figure
    :param coords tuple: Coordinates in the field where the figure is to be placed
    :param friendly str: Character representing the friendly figure
    :param enemy str: Character representing the enemy figure
    :return bool: True if the figure can be placed at the given coordinates, False otherwise
    """
    x, y = coords
    figure_height = len(figure)
    figure_width = max(len(row) for row in figure)
        ...
        for j in range(figure_width):
            if figure[i][j] == '*':
                if field[x + i][y + j] == friendly:
                    allied_overlap += 1
                if x + i >= len(field) or y + j >= len(field[0]) \
or field[x + i][y + j] == enemy or allied_overlap > 1:
                    return False
    if allied_overlap == 0:
        return False
    return True
...
def compute_move(player: int, field: list, figure: list):
    ...
    :param player int: Represents the player number
    :param field list: 2D array representing the game field
    :param figure list: 2D array representing the figure
    :return tuple: Coordinates of the next move
    """
    ...
    for i in range(len(field)):
        for j in range(len(field[0])):
            ...
            if valid_borders(field, figure, (i, j), friendly_fig, enemy_fig) \
and valid_placement(field, figure, (i, j), friendly_fig, enemy_fig):
                vacant_cells.append((i, j))

    for free_coords in vacant_cells:
        for enemy_coords in enemy_cells:
            distance = math.sqrt((free_coords[0] - enemy_coords[0]) ** 2 + (free_coords[1] - enemy_coords[1]) ** 2)
            if distance < min_distance:
                min_distance = distance
                min_distance_coords = free_coords

    return min_distance_coords if min_distance_coords is not None else (0, 0)
